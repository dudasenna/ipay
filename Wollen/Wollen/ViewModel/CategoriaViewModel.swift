...
class CategoriaViewModel: ObservableObject {
    static let shared = CategoriaViewModel()
    
    let categoriaManager = CoreDataManager.instance
    ...
    func addCategoria(nome:String, cor:String) {
        let novaCategoria = Categoria(context: categoriaManager.context)
    ...
    func lerCategorias() {
        ...
        do {
            categorias = try categoriaManager.context.fetch(request)
        } catch let error {
    ...
    func deletarCategoria(entity: Categoria) {
        categoriaManager.context.delete(entity)
        save()
    }
